...
export default function TestimonialVideos() {
  ...
  const loadVideos = useCallback(async () => {
    ...
    try {
      ...
      console.error(err);
      setError(err.message || "Failed to load testimonial videos.");
      setVideos([]);
    } finally {
      setLoading(false);
    }
  ...
  if (loading) {
    return (
      <section className="space-y-3 pt-2" aria-label="Testimonial videos">
        <div className="ui-section-header-videos">
          <CcmIcon name="video" size={18} className="text-violet-600" />
    ...
    <section className="space-y-3 pt-2" aria-label="Testimonial videos">
      <div>
        ...
        <p className="mt-1 text-xs text-slate-600">
          Swipe to watch customer stories.
        </p>
      </div>
